...
class MasterHeap
{
        // We need this, because Heap is private.
     ...
     *
     * The Block class is only the header for an allocated Block; the actual
     * memory comes after the Block object.  This class contains information
     * like the size of the memory block and the next and previous blocks in the
     * linked list for the Segment.
     *
     * This class is theoretically portable to all system archetectures, 32-bit
     * or 64-bit.
     *
     * @version 0.1
     * @author  Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
     ...
     * @since   0.1
     *
     * @note All members of this class are equal in size to the natural size of
     * the system.  Because this means it will be aligned properly no matter
     * what, because the greatest necessary alignment is the natural boundary.
     * The memory after this header Block is therefor guarranteed to be aligned
     * properly, too.
     *
     * @note This class should never be seen by the user.  It is a helper class
     * for Heap.
     */
    class Block
    {
      ...
      public:
        ...
        /** 
         * Returns the offset of the memory from the Block start.  This is
         * always equal to the sizeof the Block.
         *
         * @author Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
         * @date   2010-02-06
         * @since  0.1
      ...
      private:
        ...
        Heap  *heap;                /**< The Heap of this Block.  */
            // This alignment works because Block * and std::size_t are
            // guarranteed to be of the largest aligmnent size on a system
            // (either 32-bits or 64-bits depending on the architecture).
    };

    /**
     * A contiguous segment of memory in which Block objects are allocated.
     * Several of these make up a single Heap.  A Segment contains a linked list
     * of Blocks.
     *
     * To allocate memory, the Block calculates a place to fit the block
     * starting from the beginning of the Segment's memory.  It updates the
     * linked list accordingly.  If the memory can't be allocated, it fails and
     * returns false.  If the requested memory is greater than the size of the
     * Segment, then an OutOfMemory exception is thrown, because no Segment can
     * hold that size.
     *
     * To free memory, the Block takes the memory pointer to free and calculates
     * the location of the Block.  It then takes deletes the Block and resets
    ...
    class Segment
    {
      ...
      public:
        ...
        /** 
         * Allocates a new Block in the Segment, then sets an input pointer to
         * the Block's actual memory.
         * 
         * @author Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
         * @date   2010-02-06
         * @since  0.1
         ...
         * 
         * @return Whether the old memory was successfully freed from the
         * segment.
         ...
         *
         * @warning This method does not check to make sure the pointer passed
         * in is actually allocated.  It checks whether the memory is within
         * Segment boundaries using the IsInSegment() method.
         */
         ...
         *
         * @note There is no SetPrevious() method, because Segments cannot be
         * freed while the heap is in use with the current implementation.
         * 
         * @warning Do not call this method yourself.  It is only needed when
         * the Heap is expanding in size.
         */
      ...
      private:
        ...
        char memory[SEGMENT_SIZE];  /**< The memory for Block storage. */
        char endByte;               /**< An end byte for calculation. */
    };

    /**
     * An interface to a collection of non-contiguous Segment objects through
     * which dynamic memory allocation is provided.  Each Heap is expandable;
     * new Segment objects are automatically created when there is no longer any
     * room in the Heap for a requested allocation.  Segment objects are stored
     * in a linked list.
     *
     * The Heap class works in conjunction with the Object::new and
     * Object::delete methods to provide a transparent memory allocation method
     * that works more quickly than the system memory heap does.  Certain
     * subclasses of Object, such as Character, use a Pool to increase
     * efficiency in allocation.
     *
     * New Heap objects should not be created by the user.  During the program's
     * initialization, a Heap is created for every processor or processor core
     * on the system, allowing only one processor to be locked during a memory
     * allocation attempt.
     *
     * @version 0.1
     * @author  Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
     ...
     * @since   0.1
     *
     * @note This class should never be seen by the user.  The Object::new
     * method and Object::delete method provide the user level interface.
     * @note The maximum allocation size allowed by the Heap is equal to 50,000
     * pointer objects.
     * @note Segments are currently not able to be removed from the Heap when
     * they become empty.
     */
    class Heap
    { 
      public:
         ...
         * @since  0.1
         * 
         * @param head The first Segment from which to construct the Heap.  This
         * is Allocated by MasterHeap::AllocateSegment.
         */
        Heap (Segment *head);
        /** 
         ...
         * @since  0.1
         * 
         * @param oldMemory [in, out] A pointer to the pointer to already
        ...
        /** 
         * Chooses the Heap from which the memory to free is on (if any), and
         * then call that Heap::Free method.
         * 
         * @author Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
         * @date   2010-02-06
         * @since  0.1
  ...
  public:
    ...
    /** 
     * Returns whether the memory is on any Heap, regardless of which.  It
    ...
    /**
     * Returns the count of Heaps allocated.  This should equal the number of
     * processors/cores on the system.
     * 
     * @author Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
     * @date   2010-07-26
     * @since  0.3
     *
     * @return The number of heaps allocated on the MasterHeap.
     */
    const hummstrumm::engine::types::Number GetHeapCount (void)
      const throw ();
  ...
  private:
     ...
     * @since  0.1
     *
     * @param previous [in, out] The Segment before this one on the Heap.
     *
     ...
     * @since  0.1
     *
     * @param segment [in, out] The Segment to free.
     */ 
     ...
     * processors/cores on the system.
     *
     * This is the same as the GetHeapCount() method, but it doesn't use
     * hummstrumm::engine::types::Number, to avoid an infinite loop brought on
     * by the Object/Type System.
     * 
     * @author Patrick M. Niedzielski <PatrickNiedzielski@gmail.com>
     * @date   2010-07-27
     * @since  0.3
     ...
     * @return The number of heaps allocated on the MasterHeap.
     */
    const unsigned int GetNumberOfHeapsInternal (void)
      const throw ();
